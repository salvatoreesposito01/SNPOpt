  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        h5("Selection method"),
        selectInput(
          "search_method", "Discovery optimizer",
          choices = c("Hybrid (greedy + local search)", "GA"),
          selected = "Hybrid (greedy + local search)"
        ),
        conditionalPanel(
          condition = "input.search_method == 'GA'",
        ...
        conditionalPanel(
          condition = "input.search_method != 'GA'",
          tags$small("Hybrid mode ignores GA parameters and is usually faster.")
        ),
        ...
        downloadButton("dlFlankSkipped", "Download skipped SNPs")
      ),
